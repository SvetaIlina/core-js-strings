...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, pos).concat(str.slice(pos + value.length));
}
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replaceAll(/[\s\W]/g, '');
...
function findLongestWord(sentence) {
  const arr = sentence.split(' ').map((i) => i.length);
  const longest = Math.max(...arr);
  const index = arr.indexOf(longest);
